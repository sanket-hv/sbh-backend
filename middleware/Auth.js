...
const jwtExpirySeconds = '1D' //86400 second for 1 day..

const jwtKeyRefresh = process.env.JWT_REFRESH_SECRET_KEY
const jwtExpirySecondsRefresh = '1Y' //86400 second for 1 year..
    ...
    .then((result) => {
      if (result) {
        ...
        if (isMatch) {
          ...
          if (result.userType == 'user' && !result.status) {
            return apiResponses.unauthorizedResponse(
              res,
              `User account is expired.\nPlease contact system administrator for further queries.`,
            )
          }
  ...
  if (!token) {
    ...
    if (token.startsWith('Bearer ')) {
      ...
      try {
        ...
        if (user.userType == 'user' && !user.status) {
          return apiResponses.unauthorizedResponse(
            res,
            `User account is expired.\nPlease contact system administrator for further queries.`,
          )
